...
export default class EditHistory {
    ...
    undo() {
        ...
        const patch = this.backward.pop() as {
            redo: unknown[];
            undo: unknown[];
            forward: unknown[];
        };
        ...
        patch.undo.forEach((x) => applyChange(this.obj, x, undefined as any));
    ...
    redo() {
        ...
        const patch = this.forward.pop() as {
            redo: unknown[];
            undo: unknown[];
            forward: unknown[];
        };
        ...
        // applyChange doesn't accept arrays, only its members
        patch.redo.forEach((x) => applyChange(this.obj, x, undefined as any));
