<script lang="ts">
  ...
  function onColumnDragEnd(
    ...
    if (!__columnIndexBeingDragged)
      throw new Error('__columnIndexBeingDragged is null ');
    mColumns.splice(
  ...
  function onScroll() {
    ...
    const { scrollTop: newScrollTop, scrollLeft: newScrollLeft } =
      tableSpace as {
        scrollTop: number;
        scrollLeft: number;
        scrollWidth: number;
   ...
   */
  let visibleRows: { i: number; data: any }[];
  $: {
